...
CM33=`find ${SRC_DIR} -name r01an7254* -printf "%f\n"`
popd
...
pushd ./${LinuxBSP::-4}
BSP=`find . -name rzv_vlp* -printf "%f\n"`
echo $BSP
popd
...
tar -zxvf ${Graphics::-4}/meta-rz-features_graphics_*.tar.gz
...
unzip ${SRC_DIR}/${CM33} 
pushd ${CM33::-4}
tar -zxvf meta-rz-features_multi-os_v*.tar.gz -C $WORK_DIR
popd
rm -rfd ${CM33::-4}
...
if [[ $ISP_ENABLED -eq 1 ]]; then
	...
	cd $WORK_DIR
	unzip ${SRC_DIR}/${ISP}
	tar -zxvf ./r11an0561ej*-rzv2l-isp-sp/meta-rz-*.tar.gz
fi
